class A:
	readonly var a: bool = true # Case: initialized inline
	readonly var b: bool # Case: initialized in constructor
	readonly var c: bool # Case: not initialized	 														NOT ALLOWED
	readonly var d: bool # Case: initialized in constructor by non-pure initializer
	var e: bool = true # Control case (regression check)
	var f: bool # Control case
	readonly var g: bool = true: # Case: has setter		 												NOT ALLOWED
		set(value):
			g = value
	readonly var h: bool = true: # Case: has getter
		get():
			return h
	var i: bool = b # Case: accessing before initialized													NOT ALLOWED
	readonly var j: bool # Case: initialized in all branches in constructor
	readonly var k: bool # Case: initialized in not all branches in constructor							NOT ALLOWED
...
class C:
	var a:= A.new(true)
	readonly var b:= B.new()
	readonly var c:= [1,2,3]
	readonly var d:= Vector2.ZERO
	readonly var e: bool # Case: not initialized, no constructor											NOT ALLOWED

class D:
	var c:= C.new()
	readonly var c2:= C.new()
